...
DPLYR
Asking for tidyverse we will get this, 20 packages 
...
movies %>%               # start with the full dataset, and then
  filter(year >= 1965, length >= 60, votes >= 500, rating >= 9, mpaa >= "PG")   # give me only movies where the year is greater than or equal to 1965
...
movies %>%
  arrange(year)
```
Oldest movie is "Blacksmith Scene" in 1893 and the longest movie is "Cure for Insomnia, The"
...
movies_Ques1
```
1248 movies released in 1995  

2. Which Comedies with an MPAA rating, over 120 minutes have a rating above 8?
```{r}
movies_Ques2 <- movies %>%
  filter(mpaa != "",Comedy == 1, length > 120, rating > 8)
movies_Ques2
```
4 movies were shown titled, "Big Fish", "Forrest Gump", "Homem Que Copiava, O", and "Incredibles, The"  
...
movies_Ques3
```
This data includes three movies: Harry Potter and the Chamber of Secrets, Harry Potter and the Prisoner of Azkaban, and Harry Potter and the Sorcerer's Stone. The highest-rated Harry Potter movie is Harry Potter and the Prisoner of Azkaban, while the most-voted movie is Harry Potter and the Sorcerer's Stone.

4. What is the highest budget for a movie that is an Animation or Comedy with at least 100 votes?
```{r}
movies_Ques4 <- movies %>%
  filter(votes >= 100, Animation == 1 | Comedy == 1) %>%
    arrange(desc(budget))
movies_Ques4
      
```Star 
The movie, "Wild Wild West" has the highest budget that is a Comedy or a Animation, with at least 100 votes.

5. Is your favorite movie in the dataset? What other questions could you ask or answer using `filter()`, `select()`, and `arrange()`?

Yes, Polar Express and is my favorite movie from the data set


We could ask questions using the "or" function and arrange by rating. 
